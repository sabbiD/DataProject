function createLines(dataset, xName, yObjs, axisLables) {
    
    //var yObjs = Object.keys(yObjs)
    //console.log(yObjs)
    //console.log(yObjs[1])
...
// Object instead of array
    ...
    for (var y  in yObjs) {
        //yObjs[y] = y;
        console.log(yObjs[y])
        yObjs[y].yFunct = getYFn(yObjs[y]); //Need this  list for the ymax function
...
//Create scale functions
    ...
    .domain(d3.extent(0, dataset)); //< Can be overridden in definition
...
// Get the max of every yFunct
    ...
    /*.domain([0, d3.max(chartObj.yFuncts
    .map(chartObj.data))]);*/
...
// Render the chart
    chartObj.render = function () {
        ...
        .attr("y", 30)
        .style("text-anchor", "middle").text(chartObj.xAxisLable);
        ...
        .attr("dy", ".71em")
        .style("text-anchor", "middle").text(chartObj.yAxisLable);

        //Draw tooltips
       //var focus = chartObj.svg.append("g").attr("class", "focus").style("display", "none");

        /*for (var y  in yObjs) {
            yObjs.tooltip = focus.append("g");
            yObjs.tooltip.append("circle").attr("r", 5);
            yObjs.tooltip.append("rect")
            .attr("x", 8)
            .attr("y","-5")
            .attr("width",22)
            .attr("height",'0.75em');
            yObjs.tooltip.append("text").attr("x", 9).attr("dy", ".35em");
        }
